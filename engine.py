...
def damage_assessment(img_256, model):  #idem pour le type de dommage
	...
	pred = model.predict(img_256)
	pred_label = np.argmax(pred, axis=1)
	d = {2:'bumper_dent', 3:'bumper_scratch', 6:'door_dent', 5:'door_scratch', 7:'glass_shatter', 4:'head_lamp', 1:'tail_lamp', 0:'unknown' }
	for key in d.keys():
		if pred_label[0] == key:
			return d[key]
	# 		print "Assessment: {} damage to vehicle".format(d[key])
...
def load(img_path):
    ...
    if g2 is False:
        ...
        'gate2_result': 0,
        'gate2_message': {0: 'Are you sure that your car is damaged? Please retry your submission.',1: 'Hint: Try zooming in/out, using a different angle or different lighting.'},
        'location': None,
        'severity': None,
        'damage': None,
        'final': 'Damage assessment unsuccessful!'}
